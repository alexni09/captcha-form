...
const captchaReload = async () => {
    ...
    form.captcha = ''
    await axios.get('/api/captchareload').then(response => { htmlImgSrc.value = response.data.url }).catch(error => { console.log(error) })
}
const submit = () => {
...
<template>
    <div class="p-6">
        ...
        <form class="flex flex-col w-96" @submit.prevent="submit">
            ...
            <input type="text" v-model="form.captcha"><!-- Warning: pressing the "enter" key here reloads the captcha and clears this field. -->
